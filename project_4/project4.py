...
"""
Prim's Algorithm
This function solve the MST problem using Prim Algorithm
Input: adjacency list and adjacency matrix of the map
Output: None
"""
def prim(adjList, adjMat):
    '''
    start set to be adjList[0](cost be 0, prev be itself)
    initialize minheap
    pop one from minheap
    mark as visited
    update all its neighbors (cost, prev)
    '''
    ...
    # find one min value in priority queue at a time
    while not pq.isEmpty():
        curr = pq.deleteMin()
        curr.visited = True
        # update all unvisited neighbor of the current vertex
        for n in curr.neigh:
            if not n.visited and n.cost > adjMat[curr.rank][n.rank]:
...
This function finds the solution for MST problem using Kruskal algorithm
Input: adjacency list and sorted edge list for a graph
Output: a list of edges included in the MST solution
"""
def kruskal(adjList, edgeList):
    '''
    find the shortest unvisited edge in a graph each time
        if the two vertices in the edge already connected before, continue
        else union the two vertices and add the edge to the result

    loop until it finds |V| - 1 edges
    '''
    X = []
    ...
    # loop until there is |V| - 1 edges
    for edge in edgeList:
        v, w = edge.vertices
        # both vertices are in the same set, unvalid edge
        if (not find(v).isEqual(find(w))):
            X.append(edge)
            union(v, w)
    return X
...
"""
TSP
This function outputs a suboptimal solution for TSP problem, which is less than two times
the optimal solution
Input: adjacency list(the vertex already connected their neighbors in MST)
       a start vertex
Output: the order of vertices along the tour for the TSP problem
"""
def tsp(adjList, start):
    '''
    take start vertex as the root and explore the MST in DFS order
    '''
    tour = []
    ...
    while (s):
        ...
        # not visited, add to the tour
        if (not v.visited):
            tour.append(v.rank)
            v.visited = True
            # add all its mst neighbor on the stack
            for n in v.mstN:
                s.append(n)

    # make sure it is a round tour
...
"""
def find(v):
    # if current vertex is not the root
    # find the root and update the vertex's parent
...
def union(u,v):
    ...
    if (r1.isEqual(r2)):
        return
    # same height, the root increase height by 1
    if (r1.height == r2.height):
        r2.pi = r1.pi
        r1.height += 1
    # different height, root of height does not increase
    else:
        # make sure the heigh of r1 is larger
        if (r1.height < r2.height):
            r1, r2 = r2, r1
        r2.pi = r1
